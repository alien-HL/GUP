...
class AdvCfg():
    ...
    def get_dataset_cfg(self):
        # 数据集配置
        data_cfg = dict()
        data_cfg['dataset'] = "GUP.av1_dataset:ArgoDataset"
    ...
    def get_loss_cfg(self):
        loss_cfg = dict()
        loss_cfg["loss_fn"] = "GUP.av1_loss_fn:LossFunc"
        loss_cfg["cls_coef"] = 0.1
        loss_cfg["reg_coef"] = 0.7
        loss_cfg["reg_coef_final"] = 0.3
        loss_cfg["mgn"] = 0.2
        loss_cfg["cls_th"] = 2.0
    ...
    def get_opt_cfg(self):
        ...
        if opt_cfg['scheduler'] == 'cosine':
            ...
            opt_cfg['milestones'] = [0, 5, 30, 40, 50]
            opt_cfg['values'] = [5e-5, 5e-4, 3e-4, 2e-5, 0.5e-5]
